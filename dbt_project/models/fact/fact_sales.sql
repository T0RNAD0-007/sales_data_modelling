...
JOIN {{source('DEV_DIM', 'dim_orders')}} as do on COALESCE(cs.order_id,'1') = COALESCE(do.ordered_id,'1')
    AND COALESCE(cs.asin_code,'1') = COALESCE(do.asin_code,'1') AND COALESCE(cs.order_status,'1') = COALESCE(do.order_status,'1')
    AND COALESCE(cs.order_fulfilled_by,'1') = COALESCE(do.order_fulfilled_by,'1') AND COALESCE(cs.b2b,'1') = COALESCE(do.b2b,'1')
JOIN {{source('DEV_DIM', 'dim_products')}} as dp on COALESCE(cs.product_sku,'1') = COALESCE(dp.product_sku,'1') AND
    COALESCE(cs.product_style,'1') = COALESCE(dp.product_style,'1')
